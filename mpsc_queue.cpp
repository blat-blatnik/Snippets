...
void enqueue(Queue *queue, int item) {
	uint32_t ticket = queue->write_ticket.fetch_add(1, std::memory_order::relaxed); // Serialization with writers. 
	uint32_t slot = ticket % CAPACITY;
	uint8_t turn = (uint8_t)(ticket / CAPACITY);

	uint8_t current_turn;
	while ((current_turn = queue->slots[slot].turn.load(std::memory_order::acquire)) != turn) // Serialization with reader.
		queue->slots[slot].turn.wait(current_turn, std::memory_order::acquire);
	
	queue->slots[slot].item = item;
	queue->slots[slot].full.store(true, std::memory_order::release); // Serialization with reader.
...
int dequeue(Queue *queue) {
	...
	uint32_t slot = ticket % CAPACITY;
	uint8_t turn = (uint8_t)(ticket / CAPACITY);
	queue->slots[slot].full.wait(false, std::memory_order::acquire);
	int item = queue->slots[slot].item;
	queue->slots[slot].full.store(false, std::memory_order::relaxed);
	queue->slots[slot].turn.store(turn + 1, std::memory_order::release); // Serialization with 1 writer.
...
bool try_enqueue(Queue *queue, int item) {
	uint32_t try_ticket = queue->write_ticket.load(std::memory_order::relaxed); // Serialization with writers.
	for (;;) {
		uint32_t slot = try_ticket % CAPACITY;
		uint8_t turn = (uint8_t)(try_ticket / CAPACITY);
		uint8_t current_turn = queue->slots[slot].turn.load(std::memory_order::acquire); // Serialization with reader.
		int turns_remaining = (int)turn - (int)current_turn;
		if (turns_remaining > 0)
			return false; // Queue is full.
		else if (turns_remaining < 0)
			try_ticket = queue->write_ticket; // Turn increased in between us getting the ticket and now: someone lapped us.
...
bool try_dequeue(Queue *queue, int *out_item) {
	uint32_t ticket = queue->read_ticket;
	uint32_t slot = ticket % CAPACITY;
	if (!queue->slots[slot].full.load(std::memory_order::acquire)) // Serialization with 1 writer.
		return false;
